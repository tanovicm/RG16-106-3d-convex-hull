...
Plain* get_neighbor(Plain* plain, Point* p, Point* q)
{
	...
	return 0; // DBJ: not sure if this is OK, but it seems to work
}
...
void arrange_points(Node* plains, Node* points)
{
	for (Node* temp = points; temp != NULL; temp = temp->next) {
		...
		// DBJ: be use not to shadow the external temp
		for (Node* temp_inner = plains; temp_inner != NULL; temp_inner = temp_inner->next) {

			Plain* plain = (Plain*)temp_inner->elem;
...
Node* initial_convex_hull(Node* points)
{
	...
	for (Node* temp = points; temp != NULL; temp = temp->next) {
		// DBJ: here be sure point does not hide previous 
		// point decalaration, thus we use point_inner name
		Point* point_inner = temp->elem;

		double dist = point_to_line_distance(*point_inner, *a, *b);
	...
	// DBJ: make sure all three a,b, and c are initalized 
	// before entering make_plain!
	// otherwise app will trun and show the window 
	// and user will be able to select numer of point from context menu
	// and just then runtime will throw an error, not before!
	...
	Point* abc[] = { a,b,c };
	for (i = 0; i < 3; i++) {
		// DBJ: here be sure plain does not hide previous 
// point decalaration, thus we use plain_inner name
		Plain* plain_inner = make_plain(top, abc[i], abc[(i + 1) % 3]);
		list = add_node(list, plain_inner);
	}
...
//      now creates list of new plains and their neighbors
		...
		for (Node* i = new_plains_neighbors; i != NULL; i = i->next) {
			// DBJ: plain renamed to plain_inner
			// but it was never used?
			// Plain* plain_inner = (Plain*)i->elem;
			add_neighbors(i->elem, new_plains);
		}
